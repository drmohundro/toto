...
context Toto do
  ...
  context 'GET /' do
    ...
    asserts('body is not empty')            { !topic.body.empty? }
    ...
    context 'with a user-defined to_html' do
      ...
      asserts('Etag header has a value')      { !topic.headers['ETag'].empty? }
    end
  end
  ...
  context 'creating an article' do
    setup do
      ...
      @config[:date] = -> (t) { "the time is #{t.strftime('%Y/%m/%d %H:%M')}" }
